...
IF /I NOT "%1" == "skip-build" (
  ...
  echo ""
  echo "$COMPONENT_NAME"
  s2i build ../. -r $GIT_BRANCH $REDHAT_BASE_IMAGE $COMPONENT_NAME -e ARTIFACT_DIR=$PATH_TO_ARTIFACT_DIR
  echo "$COMPONENT_NAME docker image built succesfully"
  echo ""
  echo "$COMPONENT2_NAME"
  s2i build ../. -r $GIT_BRANCH $REDHAT_BASE_IMAGE $COMPONENT2_NAME -e ARTIFACT_DIR=$PATH_TO_ARTIFACT_DIR
